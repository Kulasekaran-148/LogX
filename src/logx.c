...
const char *log_level_to_string(log_level_t level) {
  ...
  case LOG_LEVEL_BANNER: return "INF";
...
static int check_and_rotate_locked(logx_t *l) {
  if (!l || !l->cfg.enable_file_logging || !l->cfg.file_path) return 0;
...
logx_t *logx_create(const logx_cfg_t *cfg) {
  ...
  /* copy cfg shallowly; copy strings by pointer - caller must keep them alive or they can be
   * duplicated by caller */
...
int logx_rotate_now(logx_t *logger) {
  ...
  if (logger->cfg.enable_file_logging && logger->cfg.file_path) {
    if (logger->fd >= 0) { file_lock_ex(logger->fd); }

    if (logger->fp) { fflush(logger->fp); }
    ...
    if (logger->fp) { logger->fd = fileno(logger->fp); }

    if (logger->fd >= 0) { file_lock_un(logger->fd); }
  }
  pthread_mutex_unlock(&logger->lock);
  return r;
}
...
void logx_log(logx_t *logger, log_level_t level, const char *file, const char *func, int line,
  ...
  if (use_color) {
    ...
    case LOG_LEVEL_ERROR: c = COLOR_ERROR; break;
    case LOG_LEVEL_BANNER: c = COLOR_BANNER; break;
    case LOG_LEVEL_FATAL: c = COLOR_FATAL; break;
  ...
  if (level == LOG_LEVEL_BANNER) {
    ...
    if (padded_len > sizeof(border) - 1) padded_len = sizeof(border) - 1;
  ...
  snprintf(linebuf, sizeof(linebuf), "[%s] [%s] (%s:%s:%d): ", ts, log_level_to_string(level),
  ...
  if (write_console) {
    ...
    if (logger->cfg.use_tty_detection) use_color = use_color && isatty(fileno(out));
